...
function generateMensProducts(product) {
  ...
  containerDiv.className = "womens-product";
  ...
  img.className = "womens-jacket-img";
  productLink.appendChild(img);

  const productTitle = document.createElement("p");
  productTitle.textContent = product.title;
  productTitle.className = "womens-title";
...
async function displayMensProducts(products) {
  const mensWrapper = document.querySelector("#womens-wrapper");
  mensWrapper.innerHTML = "";

  const mensProducts = products.filter(product => product.gender === "Female");
